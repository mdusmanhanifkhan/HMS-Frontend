      ...
      try {
        ...
        if (res.ok && result?.data) {
          ...
          const deptIds: number[] =
            doctor.departmentLinks?.map(
              (link: DepartmentLink) => link.department.id
            ) ?? []

          setForm({
            status: doctor.status ?? true,
            name: doctor.name ?? '',
            guardianName: doctor.guardianName ?? '',
            ...
            subSpecialities: doctor.subSpecialities ?? '',
            experience: doctor.experience ?? '',
            languages: doctor.languages ?? '',
            joinDate: doctor.joinDate ?? '',
            departmentIds: deptIds,
            employmentType: doctor.employmentType ?? null,
            availableDays: doctor.availableDays ?? [],
            timingFrom: doctor.timingFrom ?? '',
            timingTo: doctor.timingTo ?? '',
            shiftType: doctor.shiftType ?? null,
