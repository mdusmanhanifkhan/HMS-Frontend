...
interface GRN {
  id: number // Unique GRN ID
  grnNo: string // GRN number/code
  grnDate: string // Date of GRN creation
  poId?: number // Related Purchase Order ID
  createdBy?: Option // User who created the GRN
  approvedBy?: Option // User who approved the GRN (optional)
  approvedAt?: string | null // Approval timestamp (optional)
  // items: GRNItem[]           // List of items received in this GRN
  status: 'PENDING' | 'APPROVED' | 'REJECTED' | string // GRN status
...
const ApprovedPOListGrn = () => {
  ...
  const canCreateGRN = (po: PO) => {
    ...
    const allowedStatuses = [
      ...
      'ADVANCE',
      'WITHIN_30_DAYS',
      'Full payment after receiving goods',
              ...
              pos.map((po) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 flex gap-2 justify-end">
                    ...
                    <td className="px-6 py-4 flex gap-2 justify-end">
                      {po.status === 'CANCELLED' ? (
                        <Button className="px-3 py-1 rounded-full text-white text-sm font-semibold bg-red">
                          Cancelled
                        </Button>
                      ) : canCreateGRN(po) ? (
                        <Button
