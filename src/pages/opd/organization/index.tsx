...
const Organization = () => {
  ...
  const filteredOrganizations = organizations.filter((org) =>
    `${org.name} ${org.email} ${org.phone} ${org.website || ''}`
      .toLowerCase()
    ...
    <div className="flex flex-col gap-10 relative">
      ...
      <div className="flex justify-between items-center w-full border-b pb-3">
        ...
        <div className="flex items-center gap-5 min-w-100">
          ...
          <Button asLink to={routePaths.ADD_ORGANIZATION}>
            + Add Organization
          </Button>
        </div>
      </div>
          ...
          <tbody>
            ...
            {!loading &&
              ...
              filteredOrganizations.map((org) => (
                <tr key={org.id} className="bg-[#DFDEDE] border-b border-gray-200">
                  <td className="px-6 py-4 font-medium">{org.id}</td>
                    ...
                    <div className="flex items-center gap-1">
                      <span
                        className={`w-[10px] h-[10px] rounded-full ${
                          org.status ? 'bg-[#00cc00]' : 'bg-[#cc0000]'
                        }`}
                      />
                  ...
                  <td className="px-6 py-4 flex items-center gap-2">
                    {/* Edit */}
                    <Link
                      to={`${routePaths.EDIT_ORGANIZATION}/${org.id}`}
